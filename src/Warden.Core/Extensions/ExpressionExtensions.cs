...
public static class ExpressionExtensions
{
    ...
    /// </code>
    /// </example>
    public static Expression<T> CombineExpressions<T>(
        this Expression<T> firstExpression,
        Expression<T> secondExpression,
        Func<Expression, Expression, Expression> combineFunction
    )
    {
        var parameterMap = firstExpression
            .Parameters.Select(
                (firstParam, i) => new { firstParam, secondParam = secondExpression.Parameters[i] }
            )
            .ToDictionary(p => p.secondParam, p => p.firstParam);
        Expression mappedSecondBody = ParameterRebindVisitor.ReplaceParameters(
            parameterMap,
            secondExpression.Body
        );
        return Expression.Lambda<T>(
            combineFunction(firstExpression.Body, mappedSecondBody),
