...
internal static class ConversionHelper
{
    ...
    public static T? ToNullable<T>(
        ...
        where T : struct
    {
        return value.TryConvert(tryParse, out var result)
            ? result.Value
            : defaultValueFunc?.Invoke();
    }
        ...
        where T : struct
    {
        return value.TryConvert(tryParse, out var result)
            ? result.Value
            : defaultValueFunc?.Invoke() ?? default;
    }
    ...
    public static bool IsType<T>(this string? value, TryParseDelegate<T> tryParse)
        where T : struct
    {
        if (value is null)
            return false;
