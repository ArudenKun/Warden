...
public static class EnumExtensions
{
        ...
        where T : struct, Enum
    {
        if (!Enum.IsDefined(typeof(T), itemValue))
            throw new InvalidOperationException(
                $"Value {itemValue} is not defined for enum {typeof(T).Name}"
            );
    ...
    public static bool TryGetEnum<T>(this string? itemName, out T value)
        where T : struct, Enum
    {
        value = default;
        if (string.IsNullOrEmpty(itemName))
    ...
    public static bool TryGetEnum<T>(this int itemValue, out T value)
        where T : struct, Enum
    {
        value = default;
