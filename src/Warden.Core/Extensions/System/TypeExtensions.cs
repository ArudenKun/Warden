...
public static class TypeExtensions
{
    ...
    /// </summary>
    private static readonly ConcurrentDictionary<Type, IEnumerable<ColumnInfo>> ColumnInfoCache =
        new();
    ...
    public static string GetTypeName(this Type t, bool flag = true)
    {
        if (t.IsGenericType)
        {
            ...
            var result =
                $"{t.Name.Remove(t.Name.IndexOf('`'))}<{string.Join(",", t.GetGenericArguments().Select(at => at.GetTypeName()))}>";
            return result;
        }
    ...
    public static T? GetDescriptionValue<T>(this Type field)
        where T : Attribute
    {
    ...
    public static bool HasAttribute<TAttr>(this Type type, Func<TAttr, bool> predicate)
    ...
    public static IEnumerable<PropertyInfo> AttrProps<TAttribute>(this Type type)
        where TAttribute : Attribute
    {
        return type.AttrProps(typeof(TAttribute));
    }
    ...
    public static Dictionary<PropertyInfo, TAttribute?> AttrValues<TAttribute>(this Type type)
        where TAttribute : Attribute
#else
    public static Dictionary<PropertyInfo, E> AttrValues<E>(this Type type)
        where E : Attribute
#endif
    {
        return type.AttrPropValues<TAttribute>();
    }
    ...
    /// <returns>A dictionary of properties and their attribute values.</returns>
#if NET5_0_OR_GREATER
    public static Dictionary<PropertyInfo, TAttribute?> AttrPropValues<TAttribute>(this Type type)
        where TAttribute : Attribute
#else
    public static Dictionary<PropertyInfo, E> AttrPropValues<E>(this Type type)
        where E : Attribute
#endif
    {
        IEnumerable<PropertyInfo> props = type.AttrProps<TAttribute>();
        return props.ToDictionary(item => item, item => item.GetCustomAttribute<TAttribute>());
    }
#endif
    ...
    public static PropertyInfo GetPropertyInfo(this Type objType, string name)
    {
        ...
        return matchedProperty
            ?? throw new InvalidOperationException(
                $"Property '{name}' not found on type '{objType.FullName}'."
            );
    }
    ...
    public static IEnumerable<ColumnInfo> GetColumnsInfo(this Type type)
    {
        ...
        return ColumnInfoCache.GetOrAdd(
            type,
            t =>
            {
                ...
                foreach (var propertyInfo in properties)
                {
                    ...
                    var attribute =
                        propertyInfo.GetCustomAttribute<UserDefinedTableTypeColumnAttribute>(true);
