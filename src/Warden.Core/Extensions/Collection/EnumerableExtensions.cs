...
public static partial class EnumerableExtensions
{
    ...
    ) // where T : new()
    {
        ...
        Type type = typeof(T);
        var columns = type.GetColumnsInfo().ToArray();
    ...
    private static DataTable AddColumns(
        ...
        Action<DataColumn, ColumnInfo>? action = null
    )
    {
    ...
    /// </code>
    /// </example>
    private static DataTable AddRows<T>(
        this DataTable dt,
        IEnumerable<ColumnInfo> columns,
        IEnumerable<T> list,
        Action<DataRow, ColumnInfo, T>? action = null
    ) //where T : new()
    {
    ...
    )
    {
        ...
        var formattedList = self.Select(item =>
            format?.Invoke(item) ?? item?.ToString() ?? string.Empty
        );
