...
public static partial class StringExtensions
{
    ...
    [return: NotNullIfNotNull(nameof(str))]
    public static string? RemovePrefixAndSuffix(
        this string? str,
        string value,
    ...
    )
    {
        ...
        if (startIndex == 0 && endLength == str.Length)
            return str; // defensive
    ...
    /// </example>
    [Obsolete(
        "For single-character removal prefer RemoveLastChar(char). For multi-character exact suffix removal use RemoveSuffixOnce(string, StringComparison). The multi-character overload trims by character set (legacy).",
        false
    )]
    ...
    public static string RemoveSuffixOnce(
        this string str,
        string suffix,
    ...
    public static string EnsureStartsWith(
        ...
        string prefix,
        StringComparison comparison
    ) => EnsureAffixCore(value, prefix, true, comparison);
    ...
    public static string EnsureStartsWith(this string value, string prefix) =>
        EnsureAffixCore(value, prefix, true, null);
    ...
    public static string EnsureEndsWith(this string value, string suffix) =>
        EnsureAffixCore(value, suffix, false, null);
    ...
    public static string EnsureEndsWith(
        ...
        string suffix,
        StringComparison comparison
    ) => EnsureAffixCore(value, suffix, false, comparison);
    ...
    private static string EnsureAffixCore(
        string value,
        string affix,
        bool isPrefix,
        StringComparison? comparison
    ...
    {
        if (value == null)
            return affix ?? string.Empty;
        if (affix == null)
            return value;
        bool has = comparison.HasValue
            ? (
                isPrefix
                    ? value.StartsWith(affix, comparison.Value)
                    : value.EndsWith(affix, comparison.Value)
            )
