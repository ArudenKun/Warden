...
public static partial class StringExtensions
{
    ...
    /// <param name="encoding">The encoding to use for the conversion.</param>
    /// <returns>The resulting byte array if the conversion is successful, otherwise the default value.</returns>
    public static byte[]? ToBytesOrNull(
        this string? value,
        byte[]? defaultValue,
        Encoding encoding
    ...
    public static Guid ToGuid(this string? value) => value.ToGuid(() => Guid.Empty);
    ...
    public static Stream ToStream(this string value) => value.ToStreamOrNull() ?? Stream.Null;
    ...
    public static DateTime? ToDateTimeOrNull(
        this string? value,
        Func<DateTime?>? defaultValueFunc
    ...
    public static DateTime ToDateTimeOrDefault(
        ...
        value.TryToDateTime(out var result)
            ? result.Value
            : defaultValueFunc?.Invoke() ?? DateTime.MinValue;

    #endregion

    #region bool

    private static readonly Dictionary<string, bool> s_boolMap = new(
    ...
    private static bool TryParseBoolExtended(ReadOnlySpan<char> value, out bool result)
    {
        ...
        return s_boolMap.TryGetValue(trimmed.ToString(), out result)
            || bool.TryParse(trimmed, out result);
    }
#else
    /// <summary>
