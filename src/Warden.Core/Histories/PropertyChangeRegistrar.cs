...
internal static class PropertyChangeRegistrar
{
    ...
    public static IDisposable? Register(
        object? target,
        History? service,
        ConditionalWeakTable<object, Dictionary<string, object>>? valueCache,
        ILogger logger
    )
    {
        ...
        if (target is INotifyPropertyChanging inpcChanging)
        {
            ...
            disposables.Add(
                new DisposableAction(() => inpcChanging.PropertyChanging -= changingHandler)
            );
        }
        ...
        if (target is INotifyPropertyChanged inpc)
        {
            PropertyChangedEventHandler changedHandler = (s, e) =>
            {
                ...
                var hadOldVal = false;
                if (
                    valueCache.TryGetValue(s, out var cache)
                    && cache.TryGetValue(e.PropertyName, out var o)
                )
                {
                    oldVal = o;
                    hadOldVal = true;
                ...
                try
                {
                    ...
                    var action = ActionFactory.CreatePropertyChangeAction(
                        ...
                        oldVal,
                        newVal,
                        $"{s.GetType().Name}.{prop.Name} changed",
                        logger
                    );
