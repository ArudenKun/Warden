...
internal sealed class UndoBuffer : IUndoStack
{
    ...
    public int Push(IUndo command, int executeVersion, int undoVersion)
    {
        if (_hasOperation)
        {
            if (
                _buffer[_current].Command is IMergeableUndo mergeable
        ...
        _hasOperation = true;
        _currentState = true;
        _buffer[_current] = new Operation(command, executeVersion, undoVersion);
        _head = _current;
    ...
    public int Redo()
    {
        ...
        return operation.ExecuteVersion;
    }
