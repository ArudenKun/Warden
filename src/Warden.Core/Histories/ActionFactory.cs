...
using Warden.Core.Histories.Actions;
...
public static class ActionFactory
{
    private static readonly ConcurrentDictionary<
        string,
        Func<object, object, object, object, string, IHistoryAction>
    > PropCtorCache = new();
    ...
    public static IHistoryAction? CreatePropertyChangeAction(
        object? target,
        PropertyInfo? prop,
        object setterDelegate,
        object oldValue,
        object newValue,
        string description,
        ILogger? logger = null
    )
    {
        ...
        var ctor = PropCtorCache.GetOrAdd(
            key,
            k => BuildPropertyCtor(target.GetType(), prop.PropertyType)
        );
    ...
    /// <param name="logger">The logger for error reporting.</param>
    /// <returns>The created undoable action, or null if creation failed.</returns>
    public static IHistoryAction CreateCollectionChangeAction(
        ...
        int index,
        int toIndex,
        IEnumerable<object> clearedItems,
        string description,
        ILogger? logger = null
    )
    {
        ...
        var ctor = CollectionCtorCache.GetOrAdd(
            key,
            k => BuildCollectionCtor(collectionInstance.GetType(), elementType)
        );
        ...
        catch (Exception ex)
        {
            ...
            try
            {
                ...
                return created as IHistoryAction;
            }
            catch (Exception ex2)
            {
    ...
    private static Func<object, object, object, object, string, IHistoryAction> BuildPropertyCtor(
        Type targetType,
        Type valueType
    )
    {
        ...
        var setterCast = Expression.Convert(
            setterParam,
            typeof(Action<,>).MakeGenericType(targetType, valueType)
        );
        ...
        var castToIUndo = Expression.Convert(newExpr, typeof(IHistoryAction));

        var lambda = Expression.Lambda<
            Func<object, object, object, object, string, IHistoryAction>
        >(castToIUndo, targetParam, setterParam, oldParam, newParam, descParam);
    ...
    private static Func<
        ...
        string,
        IHistoryAction
    > BuildCollectionCtor(Type collectionRuntimeType, Type elementType)
    {
        var actionType = typeof(CollectionChangeAction<>).MakeGenericType(elementType);
        ...
        {
            if (
                p.ParameterType == typeof(CollectionChangeType)
                || p.ParameterType == typeof(CollectionChangeType?)
            )
            {
            ...
            )
            {
                ...
                var method = typeof(ActionFactory)
                    .GetMethod(
                        nameof(CreateTypedList),
