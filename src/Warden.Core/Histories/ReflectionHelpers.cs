...
internal static class ReflectionHelpers
{
    ...
    private static readonly ConcurrentDictionary<string, Action<object, object>> ObjectSetterCache =
        new();

    private static readonly ConcurrentDictionary<string, Func<object, object>> ObjectGetterCache =
        new();

    private static readonly ConcurrentDictionary<string, PropertyInfo> PropertyCache = new();
    private static readonly ConcurrentDictionary<string, PropertyInfo[]> PropertiesCache = new();
    ...
    /// <returns>The compiled setter action (boxed as object), or null if creation failed.</returns>
    public static object? CreateSetter(Type targetType, PropertyInfo prop, ILogger logger)
    {
        try
        {
            ...
            if (SetterCache.TryGetValue(key, out var existing))
            {
                return existing;
            }
            ...
            SetterCache.TryAdd(key, compiled);
    ...
    public static object CreateGetter(Type targetType, PropertyInfo prop, ILogger logger = null)
    {
        try
        {
            ...
            if (GetterCache.TryGetValue(key, out var existing))
            {
                return existing;
            }
            ...
            var compiled = lambda.Compile();
            GetterCache.TryAdd(key, compiled);
    ...
    public static Action<object, object> CreateObjectSetter(
        Type targetType,
        PropertyInfo prop,
        ILogger logger = null
    )
    {
        try
        {
            ...
            if (ObjectSetterCache.TryGetValue(key, out var existing))
            {
                return existing;
            }
            ...
            var compiled = lambda.Compile();
            ObjectSetterCache.TryAdd(key, compiled);
            return compiled;
        }
        catch (Exception ex)
        {
    ...
    public static Func<object, object> CreateObjectGetter(
        Type targetType,
        PropertyInfo prop,
        ILogger logger = null
    )
    {
        try
        {
            ...
            if (ObjectGetterCache.TryGetValue(key, out var existing))
            {
                return existing;
            }
            ...
            var compiled = lambda.Compile();
            ObjectGetterCache.TryAdd(key, compiled);
            return compiled;
        }
        catch (Exception ex)
        {
