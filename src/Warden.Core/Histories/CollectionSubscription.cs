...
internal sealed class CollectionSubscription : IDisposable
{
    ...
    )
    {
        ...
        _collectionInstance =
            collectionInstance ?? throw new ArgumentNullException(nameof(collectionInstance));
        _incc =
            collectionInstance as INotifyCollectionChanged
            ?? throw new ArgumentException(
                "collectionInstance must implement INotifyCollectionChanged"
            );
    ...
    private void CreateAndPush(
        string changeName,
        object itemObj = null,
        object oldItemObj = null,
        int index = -1,
        int toIndex = -1,
        IEnumerable<object> clearedItems = null,
        string desc = null
    )
    {
        try
            ...
            if (!_service.IsApplying)
            {
                ...
                if (changeType == CollectionChangeType.Clear)
                {
                    try
                    {
                        ...
                        if (created is IHistoryAction createdAction)
                        {
                            System.Console.WriteLine(
                                $"CreateAndPush: pushing Clear action desc={desc} on {_collectionInstance}"
                            );
                ...
                var action = ActionFactory.CreateCollectionChangeAction(
                    _collectionInstance,
                    _elementType,
                    changeType,
                    itemObj,
                ...
                if (action is IHistoryAction ua)
                {
                    try
                    {
                        System.Console.WriteLine(
                            $"CreateAndPush: ActionFactory created action {ua.Description} change={changeType} item={itemObj} index={index}"
                        );
                    }
                    catch (Exception ex)
                    {
                ...
                try
                {
                    ...
                    if (created is IHistoryAction createdAction)
                    {
                        ...
                        {
                            System.Console.WriteLine(
                                $"CreateAndPush: Activator created action {createdAction.Description} change={changeType} item={itemObj} index={index}"
                            );
                        }
                        catch (Exception ex)
                        {
        ...
        try
        {
            ...
            switch (e.Action)
            {
                    ...
                    if (e.NewItems != null)
                    {
                        ...
                        else
                        {
                            for (int i = 0; i < e.NewItems.Count; i++)
                            {
                                ...
                                _snapshot?.Insert(
                                    idx >= 0 && idx <= _snapshot.Count ? idx : _snapshot.Count,
                                    newItem
                                );
                            }
                        }
                ...
                case NotifyCollectionChangedAction.Replace:
                    ...
                    {
                        int start =
                            e.NewStartingIndex >= 0 ? e.NewStartingIndex : e.OldStartingIndex;
                        for (int i = 0; i < e.NewItems.Count; i++)
                        {
                            var newItem = e.NewItems[i];
                            var oldItem = e.OldItems[i];
